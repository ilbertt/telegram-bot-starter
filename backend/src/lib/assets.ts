...
const PUBLIC_FRONTEND_DIR = resolve(process.cwd(), publicFolderName);
const DB_MIGRATIONS_DIR = resolve(import.meta.dir, '..', 'db', migrationsFolderName);
...
export function getPublicAssets(): AssetFiles {
  return readAssets({ folderName: publicFolderName, folder: PUBLIC_FRONTEND_DIR });
}

export function getMigrations(): AssetFiles {
  return readAssets({ folderName: migrationsFolderName, folder: DB_MIGRATIONS_DIR });
}
